...
index = 0
E = 7.143e6
# E_list=np.linspace(7.1e6,7.2e6,10)
...
Force=0.3
a_0 = np.sqrt(sphere_radius*depth_indentation) #contact radius
    ...
    for i in range(size):
      ...
      # Calculate X1 and X2 for each centroid
        ...
        def U_star(d, z):
            t=z/H
            R=a_0*t
            return c*t*(1-(1/(1+np.exp(-k*(d-R))))+(1-(m*(d-R)/(L/2 - R))))
            
            
        def U_star_devX(d, z, x):
            t=z/H
            R=a_0*t
            return c*t*((-m/(L/2 -R))-(k*np.exp(-k*(d-R))/(1+np.exp(-k*(d-R)))**2))*x*d**(-1/2)

        def U_star_devY(d, z, y):
            t=z/H
            R=a_0*t
            return c*t*((-m/(L/2 -R))-(k*np.exp(-k*(d-R))/(1+np.exp(-k*(d-R)))**2))*y*d**(-1/2)
            
        def U_star_devZ(d, z):
            t=z/H
            R=a_0*t
            return c/H*(1-(1/(1+np.exp(-k*(d-R))))+(1-(m*(d-R)/(L/2 - R))))
...
def main():
    ...
    deformation_gradients = calculate_deformation_gradient(tensor_displacement_list, matrix,E,v,undeformed_centroids,cube_size)

    # phi=[]
    # for E in E_list:
    #     phi.append(calculate_deformation_gradient(tensor_displacement_list, matrix,E,v,undeformed_centroids,cube_size))
    # phi=np.array(phi)
    # np.save('phi_indentation_1.npy',phi)
